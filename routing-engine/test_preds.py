...
import numpy as np
import argparse
from ml_pipeline.train_stgnn import STGNN, load_real_colombo_graph

def check(hour):
    data, mapping = load_real_colombo_graph("data/real_traffic_edges.csv")
    device = torch.device("cpu")
    data = data.to(device)
    model = STGNN(seq_len=12, hidden_dim=64, future_steps=1).to(device)
    model.load_state_dict(torch.load("ml-pipeline/colombo_traffic_model_real.pth", map_location=device, weights_only=True))
    model.eval()
    with torch.no_grad():
        shifts = 24 - hour
